...
Contudo, um dos problemas que nos apareceu estava associado a limitação de memoria no R-Studio, ou seja, não estavamos a conseguir trabalhar com um dataset tão extenso. Posto isto, uma alternativa foi trabalhar com os 16000 genes com menor padjust e, portanto, os 16000 genes mais diferencialmente expressos. Sabemos que poderíamos optar por explorar aqui os resultados do PCA, contudo foi opção nossa utilizar a primeira abordagem.
...
Além disso foi utilizado uma cross validation com 10 folds repetida 5 vezes. Para tornar estes resultados replicáveis , foi invocado uma seed 16718. Um aspecto que também se teve em consiração foi a diferente proporção entre a classe 1- Mutante e a classe 2 WT, daí se ter utilizado a primeira linha de código descrita abaixo.

```{r}
#rerpesentação grafica da distribuição da classe 1 e 2
...
Tebdo em conta as métricas analisadas queremos salientar a XXX coo XX % sendo este o parametro que daqui em diante iremos comparar entre os modelos estudados com o intuito de escolher um melhor modelo.

### Naive Bayes
Para este modelo em particular nao foi avaliado nenhum hiperparametro em especifico , tendo sido apenas treinado o algoritmo de ML em estudo face ao dataset previamente dividido.
